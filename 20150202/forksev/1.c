...
int main(int argc, char *argv[])
{
	...
	if(pid == 0){  //child process
		...
		if((fd_send = open("./send", O_RDONLY)) == -1){
			perror("open send pipe failed");
			exit(1);
		}

		printf("child open read pipe successed\n");
		while(1){
			memset(buf, 0, 1024);
			memset(msg, 0, 128);

			if(read(fd_send, buf, 1024) != 0){
		...
		if((fd_recv = open("./recv", O_WRONLY)) == -1){
